        ...
        if 'checking_code' in request.POST:
            ...
            if user.checking_error < 4:
                if checking_code == user.checking_code:
                    ...
                    user.save()
                    baseUser.save()
                    return HttpResponseRedirect(reverse('loan:borrowing_message'))
                else:
...
class ForgotView(View):
    ...
    def post(self, request, *args, **kwargs):
        if 'checking_code' in request.POST:
            ...
            if not User.objects.filter(username=phone):
                context['message'] = '該電話未註冊'
                return render(request, self.template_name, context)
...
class BackendView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        phone = request.POST['phone']
        ad_region = request.POST['ad_region']
        ad_photo_big = request.FILES.get('ad_photo_big')
        ad_photo_small = request.FILES.get('ad_photo_small')

        user = User.objects.get(username=phone)
        Ad.objects.create(lender=user.myuser.lender, ad_region=ad_region, ad_photo_big=ad_photo_big, ad_photo_small=ad_photo_small)
